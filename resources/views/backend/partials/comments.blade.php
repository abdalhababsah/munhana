    ...
    <div class="p-6">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between mb-1">
                                ...
                                @if(auth()->id() === $comment->user_id || auth()->user()->role === 'admin')
                                    <div class="flex items-center gap-2">
                                        <button type="button" class="text-warning hover:text-warning/80 edit-comment-btn"
                                            data-comment-id="{{ $comment->id }}" title="{{ __('messages.edit') }}">
                                            <i class="uil uil-edit text-lg"></i>
                                        </button>
                                        <button type="button" class="text-danger hover:text-danger/80 delete-comment-btn"
                                            data-comment-id="{{ $comment->id }}" title="{{ __('messages.delete') }}">
                            ...
                            <div class="edit-form hidden mt-2">
                                <form class="update-comment-form">
                                    @csrf
                                    @method('PUT')
                                    <textarea name="comment" rows="3" class="form-input w-full mb-2"
        ...
        <div class="border-t pt-6">
            ...
            <form id="add-comment-form" method="POST" action="{{ route('backend.comments.store') }}">
                ...
                <div class="mb-3">
                    <textarea name="comment" id="comment-input" rows="4"
                        class="form-input w-full @error('comment') border-danger @enderror"
                        placeholder="{{ __('messages.write_your_comment') }}" required></textarea>
                    @error('comment')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            commentsSection.addEventListener('click', async function (e) {
                confirmAction(null, '{{ __("messages.confirm_delete_comment") }}', null, async () => {
            ...
            function showNotification(message, type = 'success') {
                window.dispatchEvent(new CustomEvent('open-confirmation-modal', {
                    detail: {
                        message: message,
                        title: type === 'success' ? '{{ __("messages.success") }}' : '{{ __("messages.error") }}',
                        confirmText: '{{ __("messages.ok") }}',
                        cancelText: '', // Hide cancel button
                        confirmButtonClass: type === 'success' ? 'btn-success' : 'btn-danger'
                    }
                }));
            }
        });
