                        ...
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                    {{ __('messages.date') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                    {{ __('messages.maintenance_type') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                            ...
                            @foreach($schedules as $schedule)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <button type="button" data-hs-overlay="#edit-schedule-{{ $schedule->id }}"
                                            class="btn btn-sm btn-light me-2">
                                            {{ __('messages.edit') }}
                                        </button>
                                        <form method="POST" action="{{ route('backend.maintenance-schedules.destroy', $schedule) }}"
                                            class="inline"
                                ...
                                <div id="edit-schedule-{{ $schedule->id }}"
                                    class="hs-overlay hidden w-full h-full fixed top-0 start-0 z-50 overflow-y-auto">
                                    <div
                                        class="hs-overlay-open:mt-8 hs-overlay-open:opacity-100 mt-0 opacity-0 transition-all max-w-2xl mx-auto">
                                        <div class="bg-white rounded-lg shadow-lg">
                                            <div class="flex items-center justify-between p-4 border-b">
                                                <h4 class="text-lg font-semibold">{{ __('messages.edit_maintenance_schedule') }}
                                                </h4>
                                                <button type="button" class="text-gray-400 hover:text-gray-600"
                                                    data-hs-overlay="#edit-schedule-{{ $schedule->id }}">✕</button>
                                            </div>
                                            <form method="POST"
                                                action="{{ route('backend.maintenance-schedules.update', $schedule) }}"
                                                class="p-6 space-y-4">
                                                @csrf
                                                @method('PUT')
                                                <div class="grid md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <input type="date" name="maintenance_date"
                                                            value="{{ $schedule->maintenance_date?->format('Y-m-d') }}"
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="text" name="maintenance_type"
                                                            value="{{ $schedule->maintenance_type }}" class="form-input w-full"
                                                            required>
                                                    </div>
                                                    <div>
                                                        <label class="form-label">{{ __('messages.maintenance_type') }} (AR)</label>
                                                        <input type="text" name="maintenance_type_ar"
                                                            value="{{ $schedule->maintenance_type_ar }}" dir="rtl"
                                                            class="form-input w-full" required>
                                                    </div>
                                                </div>
                                                <div class="grid md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <textarea name="notes" rows="3"
                                                            class="form-textarea w-full">{{ $schedule->notes }}</textarea>
                                                    </div>
                                                    <div>
                                                        <label class="form-label">{{ __('messages.notes') }} (AR)</label>
                                                        <textarea name="notes_ar" rows="3" dir="rtl"
                                                            class="form-textarea w-full">{{ $schedule->notes_ar }}</textarea>
                                                    </div>
                                                </div>
                                                <div class="flex justify-end gap-3">
                                                    <button type="button" class="btn btn-light"
                                                        data-hs-overlay="#edit-schedule-{{ $schedule->id }}">{{ __('messages.cancel') }}</button>
                                                    <button type="submit"
