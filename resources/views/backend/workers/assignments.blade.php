                    ...
                    <form method="POST" action="{{ route('backend.workers.assignments.store', $project) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="role_description" name="role_description" class="form-input w-full"
                                    placeholder="{{ __('messages.eg_site_supervisor') }}">
                            </div>
                            <div>
                                ...
                                <input type="text" id="role_description_ar" name="role_description_ar" class="form-input w-full"
                                    placeholder="{{ __('messages.eg_site_supervisor_ar') }}" dir="rtl">
                            </div>
                        </div>
                        ...
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">
                                    {{ __('messages.worker') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">
                                    {{ __('messages.role_description') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">
                                    {{ __('messages.assigned_by') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">
                                    {{ __('messages.assigned_date') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($project->assignedUsers as $worker)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 rounded-full bg-warning/10 flex items-center justify-center me-3">
                                                <span
                                                    class="text-sm font-semibold text-warning">{{ strtoupper(substr($worker->name, 0, 2)) }}</span>
                                            </div>
                                            <div>
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($worker->pivot->assigned_at)->format('Y-m-d') }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <button type="button" class="text-warning hover:text-warning/80"
                                                data-hs-overlay="#edit-modal-{{ $worker->id }}" title="{{ __('messages.edit') }}">
                                                <i class="uil uil-edit text-lg"></i>
                                            </button>
                                            <form method="POST"
                                                action="{{ route('backend.workers.assignments.destroy', [$project, $worker]) }}"
                                                onsubmit="return confirmAction(event, '{{ __('messages.confirm_remove_worker') }}');"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <div
                                        ...
                                        <div class="flex flex-col bg-white border shadow-sm rounded-xl">
                                            <div class="flex justify-between items-center py-3 px-4 border-b">
                                                ...
                                                <button type="button"
                                                    class="hs-dropdown-toggle inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-gray-500 hover:text-gray-400"
                                                    data-hs-overlay="#edit-modal-{{ $worker->id }}">
                                                    <i class="uil uil-times text-xl"></i>
                                                </button>
                                            </div>
                                            <form method="POST"
                                                action="{{ route('backend.workers.assignments.update', [$project, $worker]) }}">
                                                @csrf
                                                @method('PUT')
                                                <div class="p-4 space-y-4">
                                                    <div>
                                                        ...
                                                        <input type="text" name="role_description"
                                                            value="{{ $worker->pivot->role_description }}"
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="text" name="role_description_ar"
                                                            value="{{ $worker->pivot->role_description_ar }}"
                                                            class="form-input w-full" dir="rtl">
                                                    </div>
                                                </div>
