...
@section('content')
    <div class="flex flex-col gap-6">
                ...
                @if($issue->photo_path)
                    <div>
                        ...
                        <img src="{{ asset('storage/' . $issue->photo_path) }}" alt="Issue photo"
                            class="rounded-lg max-h-96 object-contain bg-gray-50">
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="p-6 space-y-4">
                <form method="POST" action="{{ route('backend.warranty-issues.update', $issue) }}"
                    ...
                    <div>
                        ...
                        <select name="status" class="form-select w-full @error('status') border-danger @enderror" required>
                            <option value="open" {{ $issue->status === 'open' ? 'selected' : '' }}>{{ __('messages.open') }}
                            </option>
                            <option value="in_progress" {{ $issue->status === 'in_progress' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea name="resolution_notes" rows="3"
                            class="form-textarea w-full @error('resolution_notes') border-danger @enderror">{{ old('resolution_notes', $issue->resolution_notes) }}</textarea>
                ...
                <form method="POST" action="{{ route('backend.warranty-issues.destroy', $issue) }}"
