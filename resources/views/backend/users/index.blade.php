                ...
                <form method="GET" action="{{ route('backend.users.index') }}" class="flex items-end gap-4">
                    <div class="flex-1">
                        ...
                        <input type="text" id="search" name="search" value="{{ request('search') }}"
                            class="form-input w-full" placeholder="{{ __('messages.search_by_name_or_email') }}">
                    </div>
                    <div class="flex-1">
                        ...
                        <select id="role" name="role" class="form-input w-full">
                            ...
                            <option value="client" {{ request('role') === 'client' ? 'selected' : '' }}>
                                {{ __('messages.client') }}</option>
                            <option value="worker" {{ request('role') === 'worker' ? 'selected' : '' }}>
                                {{ __('messages.worker') }}</option>
                        </select>
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($users as $user)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center me-3">
                                                <span
                                                    class="text-sm font-semibold text-primary">{{ strtoupper(substr($user->name, 0, 2)) }}</span>
                                            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm text-gray-600">{{ $user->language === 'ar' ? __('messages.arabic') : __('messages.english') }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('backend.users.show', $user) }}" class="text-info hover:text-info/80"
