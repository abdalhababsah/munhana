...
@section('title', __('messages.edit') . ' ' . __('messages.cost') . ' - ' . (app()->getLocale() === 'ar' ? $project->name_ar : $project->name))
@section('page-title', __('messages.edit') . ' ' . __('messages.cost'))
...
<div class="flex flex-col gap-6">
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">{{ __('messages.edit') }} {{ __('messages.cost') }}</h4>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('backend.costs.update', $cost) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date"
                               id="cost_date"
                               name="cost_date"
                               value="{{ old('cost_date', $cost->cost_date) }}"
                    ...
                    <div>
                        ...
                        <select id="cost_type"
                                ...
                                required>
                            <option value="labor" {{ old('cost_type', $cost->cost_type) === 'labor' ? 'selected' : '' }}>{{ __('messages.labor') }}</option>
                            <option value="material" {{ old('cost_type', $cost->cost_type) === 'material' ? 'selected' : '' }}>{{ __('messages.material') }}</option>
                            <option value="equipment" {{ old('cost_type', $cost->cost_type) === 'equipment' ? 'selected' : '' }}>{{ __('messages.equipment') }}</option>
                            <option value="other" {{ old('cost_type', $cost->cost_type) === 'other' ? 'selected' : '' }}>{{ __('messages.other') }}</option>
                        </select>
                        @error('cost_type')
                            <p class="text-danger text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description"
                              ...
                              required>{{ old('description', $cost->description) }}</textarea>
                    @error('description')
                        <p class="text-danger text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description_ar"
                              ...
                              rows="4"
                              dir="rtl"
                              class="form-input w-full @error('description_ar') border-danger @enderror">{{ old('description_ar', $cost->description_ar) }}</textarea>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" class="btn btn-warning">
                        ...
                        {{ __('messages.update') }} {{ __('messages.cost') }}
                    </button>
                </div>
            </form>
        </div>
