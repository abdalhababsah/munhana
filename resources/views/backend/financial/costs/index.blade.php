    ...
    <div class="flex flex-col gap-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">{{ __('messages.contract_number') }}: {{ $project->contract_number }}
                </p>
            </div>
            ...
            <div class="p-6">
                <div class="mb-4">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm text-gray-600">{{ __('messages.budget') }}:
                            {{ number_format($budget, 2) }}</span>
                    ...
                    <div class="flex items-center justify-between mt-2">
                        <span class="text-sm text-gray-600">{{ __('messages.used') }}:
                            {{ number_format($budgetUsed, 2) }}</span>
        ...
        <div class="card">
            ...
            <div class="p-6">
                <form method="GET" action="{{ route('backend.costs.index', $project) }}"
                    class="flex items-end gap-4 flex-wrap">
                    <div class="flex items-center gap-2">
                        <button type="submit" name="cost_type" value=""
                            class="btn {{ !request('cost_type') ? 'btn-primary' : 'btn-light' }}">
                            {{ __('messages.all') }}
                        </button>
                        <button type="submit" name="cost_type" value="labor"
                            class="btn {{ request('cost_type') === 'labor' ? 'btn-primary' : 'btn-light' }}">
                            {{ __('messages.labor') }}
                        </button>
                        <button type="submit" name="cost_type" value="material"
                            class="btn {{ request('cost_type') === 'material' ? 'btn-success' : 'btn-light' }}">
                            {{ __('messages.material') }}
                        </button>
                        <button type="submit" name="cost_type" value="equipment"
                            class="btn {{ request('cost_type') === 'equipment' ? 'btn-warning' : 'btn-light' }}">
                            {{ __('messages.equipment') }}
                        </button>
                        <button type="submit" name="cost_type" value="other"
                            class="btn {{ request('cost_type') === 'other' ? 'btn-info' : 'btn-light' }}">
                            {{ __('messages.other') }}
                        </button>
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($costs as $cost)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-semibold text-gray-900">{{ number_format($cost->amount, 2) }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('backend.costs.edit', $cost) }}"
                                                class="text-warning hover:text-warning/80" title="{{ __('messages.edit') }}">
                                                <i class="uil uil-edit text-lg"></i>
                                            </a>
                                            <form method="POST" action="{{ route('backend.costs.destroy', $cost) }}"
                                                onsubmit="return confirmAction(event, '{{ __('messages.confirm_delete') }}');"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
