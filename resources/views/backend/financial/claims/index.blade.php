                ...
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-success">{{ number_format($stats['total_approved'], 2) }}
                            </h3>
                        </div>
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-warning">{{ number_format($stats['pending_amount'], 2) }}
                            </h3>
                        </div>
            ...
            <div class="p-6">
                <form method="GET" action="{{ route('backend.claims.index', $project) }}" class="flex items-center gap-2">
                    ...
                    <button type="submit" name="status" value="paid"
                        class="btn {{ request('status') === 'paid' ? 'btn-info' : 'btn-light' }}">
                        {{ __('messages.paid') }}
                    </button>
                </form>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($claims as $claim)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($claim->claim_date)->format('Y-m-d') }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-semibold text-gray-900">{{ number_format($claim->amount, 2) }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('backend.claims.show', $claim) }}"
                                                class="text-info hover:text-info/80" title="{{ __('messages.view') }}">
                                            ...
                                            @if($claim->status !== 'paid')
                                                <form method="POST" action="{{ route('backend.claims.destroy', $claim) }}"
