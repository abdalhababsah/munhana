...
@section('title', __('messages.edit') . ' ' . __('messages.daily_report'))
@section('page-title', __('messages.edit') . ' ' . __('messages.daily_report'))
...
<div class="flex flex-col gap-6">
    ...
    <!-- Report Info Alert -->
    <div class="p-4 bg-info/10 text-info rounded-md flex items-center">
        <i class="uil uil-info-circle text-2xl me-3"></i>
        <div>
            <p class="font-semibold">{{ __('messages.created_by') }}: {{ $report->creator->name }}</p>
            <p class="text-sm">{{ __('messages.created_at') }}: {{ $report->created_at->format('Y-m-d H:i') }}</p>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">{{ __('messages.edit') }} {{ __('messages.daily_report') }}</h4>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('backend.reports.daily.update', $report) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number"
                               id="worker_count"
                               name="worker_count"
                               value="{{ old('worker_count', $report->worker_count) }}"
                               min="0"
                    ...
                    <div>
                        ...
                        <input type="number"
                               ...
                               value="{{ old('completion_percentage', $report->completion_percentage) }}"
                               min="0"
                               max="100"
                               step="0.1"
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea id="notes"
                                  ...
                                  placeholder="{{ __('messages.daily_work_summary') }}">{{ old('notes', $report->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea id="notes_ar"
                                  ...
                                  placeholder="{{ __('messages.daily_work_summary') }}">{{ old('notes_ar', $report->notes_ar) }}</textarea>
                        @error('notes_ar')
                            <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                        @enderror>
                    </div>
