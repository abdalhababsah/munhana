                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-gray-900">
                                {{ number_format($stats['total_value_delivered'], 2) }}</h3>
                        </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($groupedDeliveries as $boqItemId => $data)
                                ...
                                <tr class="hover:bg-gray-50 cursor-pointer"
                                    @click="expandedItems[{{ $boqItemId }}] = !expandedItems[{{ $boqItemId }}]">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-1 min-w-[120px]">
                                                <div
                                                    class="text-sm font-semibold {{ $percentage > 100 ? 'text-danger' : ($percentage === 100 ? 'text-success' : 'text-warning') }}">
                                ...
                                <tr x-show="expandedItems[{{ $boqItemId }}]" x-cloak>
                                    <td colspan="6" class="px-6 py-4 bg-gray-50">
                                        <div class="text-sm font-semibold text-gray-700 mb-3">
                                            {{ __('messages.delivery_history') }} ({{ $data['deliveries']->count() }}
                                            {{ __('messages.deliveries') }})
                                        </div>
                                        <table class="min-w-full divide-y divide-gray-300">
                                            <thead class="bg-gray-100">
                                                <tr>
                                                    <th class="px-4 py-2 text-start text-xs font-medium text-gray-600">
                                                        {{ __('messages.delivery_date') }}</th>
                                                    <th class="px-4 py-2 text-start text-xs font-medium text-gray-600">
                                                        {{ __('messages.quantity') }}</th>
                                                    <th class="px-4 py-2 text-start text-xs font-medium text-gray-600">
                                                        {{ __('messages.supplier') }}</th>
                                                    <th class="px-4 py-2 text-start text-xs font-medium text-gray-600">
                                                        {{ __('messages.received_by') }}</th>
                                                    <th class="px-4 py-2 text-start text-xs font-medium text-gray-600">
