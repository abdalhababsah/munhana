...
@section('title', __('messages.new') . ' ' . __('messages.site_visit'))
@section('page-title', __('messages.new') . ' ' . __('messages.site_visit'))
...
<div class="flex flex-col gap-6">
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">{{ __('messages.new') }} {{ __('messages.site_visit') }}</h4>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('backend.visits.store') }}">
                @csrf
                ...
                <div class="grid lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" id="visit_date" name="visit_date"
                               value="{{ old('visit_date', \Carbon\Carbon::today()->format('Y-m-d')) }}"
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea id="purpose_ar" name="purpose_ar" rows="4" dir="rtl"
                                  ...
                                  placeholder="{{ __('messages.visit_purpose_placeholder') }}">{{ old('purpose_ar') }}</textarea>
                        @error('purpose_ar')
                            <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea id="findings_ar" name="findings_ar" rows="4" dir="rtl"
                                  ...
                                  placeholder="{{ __('messages.visit_findings_placeholder') }}">{{ old('findings_ar') }}</textarea>
                        @error('findings_ar')
                            <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                        @enderror
                    </div>
