                ...
                <div class="card-header bg-primary/5">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm text-gray-600">
                            {{ \Carbon\Carbon::parse($currentPlan->week_start_date)->format('M d') }} -
                            {{ \Carbon\Carbon::parse($currentPlan->week_end_date)->format('M d, Y') }}
                        </span>
                    </div>
                </div>
                <div class="p-6">
                    ...
                    <div class="flex items-center justify-between mt-4 pt-4 border-t">
                        <span class="text-sm text-gray-500">
                            {{ __('messages.created_by') }}: {{ $currentPlan->creator->name }} -
                            {{ $currentPlan->created_at->format('Y-m-d') }}
                        </span>
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2">
                                            <h5 class="font-semibold text-gray-900">
                                                {{ \Carbon\Carbon::parse($plan->week_start_date)->format('M d') }} -
                                                {{ \Carbon\Carbon::parse($plan->week_end_date)->format('M d, Y') }}
                                            </h5>
                                        ...
                                        <p class="text-sm text-gray-500">
                                            {{ __('messages.created_by') }}: {{ $plan->creator->name }} -
                                            {{ $plan->created_at->format('Y-m-d') }}
                                        </p>
                                    </div>
                                    <div class="flex gap-2">
                                        <a href="{{ route('backend.weekly-plans.edit', $plan) }}"
