                                ...
                                @foreach($timelines as $timeline)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                {{ \Carbon\Carbon::parse($timeline->planned_start_date)->format('Y-m-d') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                {{ \Carbon\Carbon::parse($timeline->planned_end_date)->format('Y-m-d') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ $timeline->duration_days }}
                                                {{ __('messages.days') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($timeline->status === 'completed')
                                                <span
                                                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-success/10 text-success">
                                                    <i class="uil uil-check-circle me-1"></i>
                                                    {{ __('messages.completed') }}
                                                </span>
                                            @elseif($timeline->status === 'in_progress')
                                                <span
                                                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-info/10 text-info">
                                                    <i class="uil uil-sync me-1"></i>
                                                    {{ __('messages.in_progress') }}
                                                </span>
                                            @elseif($timeline->status === 'delayed')
                                                <span
                                                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-danger/10 text-danger">
                                                    <i class="uil uil-exclamation-triangle me-1"></i>
                                                    {{ __('messages.delayed') }}
                                                </span>
                                            @else
                                                <span
                                                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-warning/10 text-warning">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('backend.timeline.edit', $timeline) }}"
                                                    class="text-warning hover:text-warning-dark" title="{{ __('messages.edit') }}">
                                                    <i class="uil uil-edit text-lg"></i>
                                                </a>
                                                <form method="POST" action="{{ route('backend.timeline.destroy', $timeline) }}"
