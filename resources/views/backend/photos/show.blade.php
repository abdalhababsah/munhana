    ...
    <div class="flex flex-col gap-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">{{ __('messages.contract_number') }}:
                    {{ $photo->project->contract_number }}</p>
            </div>
            <div class="flex items-center gap-2">
                <form method="POST" action="{{ route('backend.photos.destroy', $photo) }}"
                    onsubmit="return confirmAction(event, '{{ __('messages.confirm_delete') }}');" class="inline">
                    @csrf
                    @method('DELETE')
                ...
                <div class="card">
                    <div class="p-6">
                        <div class="rounded-lg overflow-hidden bg-gray-100">
                            <img src="{{ asset('storage/' . $photo->photo_path) }}" alt="{{ __('messages.site_photo') }}"
                                class="w-full h-auto">
                        </div>
                        <div class="mt-4 flex items-center justify-between">
                            <a href="{{ asset('storage/' . $photo->photo_path) }}" download class="btn btn-sm btn-primary">
                    ...
                    <div class="p-6">
                        <div class="space-y-4">
                            ...
                            @if($photo->description)
                                <div>
                                    <p class="text-sm text-gray-500 mb-1">{{ __('messages.description') }}
                                        ({{ __('messages.english') }})</p>
                            ...
                            @if($photo->description_ar)
                                <div>
                                    <p class="text-sm text-gray-500 mb-1">{{ __('messages.description') }}
                                        ({{ __('messages.arabic') }})</p>
                                    <p class="text-sm text-gray-700 whitespace-pre-line" dir="rtl">{{ $photo->description_ar }}
                                    </p>
                                </div>
                            @endif
                                ...
                                <div class="bg-gray-50 rounded-lg p-3 space-y-1">
                                    ...
                                    @if(file_exists(storage_path('app/public/' . $photo->photo_path)))
                                        <p class="text-xs text-gray-600">
                                            ...
                                            {{ number_format(filesize(storage_path('app/public/' . $photo->photo_path)) / 1024 / 1024, 2) }}
                                            MB
                                        </p>
                                    @endif
                                </div>
